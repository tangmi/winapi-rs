...
STRUCT!{#[debug] struct D3D11_VIDEO_DECODER_SUB_SAMPLE_MAPPING_BLOCK {
    ClearSize: UINT,
    EncryptedSize: UINT,
}}
...
STRUCT!{#[debug] struct D3D11_VIDEO_DECODER_BEGIN_FRAME_CRYPTO_SESSION {
...
STRUCT!{#[debug] struct D3D11_VIDEO_PROCESSOR_STREAM_BEHAVIOR_HINT {
    Enable: BOOL,
    Width: UINT,
    Height: UINT,
    Format: DXGI_FORMAT,
...
STRUCT!{#[debug] struct D3D11_KEY_EXCHANGE_HW_PROTECTION_INPUT_DATA {
    PrivateDataSize: UINT,
    HWProtectionDataSize: UINT,
    pbInput: [BYTE; 4],
}}
STRUCT!{#[debug] struct D3D11_KEY_EXCHANGE_HW_PROTECTION_OUTPUT_DATA {
    PrivateDataSize: UINT,
...
STRUCT!{#[debug] struct D3D11_KEY_EXCHANGE_HW_PROTECTION_DATA {
